...
public class BlockSampler {
		...
		for(int iteration=0;iteration<Parameter.iterations;iteration++){
			...
			for(int d=0;d<Parameter.N_d;d++){
				...
				for(int index_word=0;index_word<document.n_d_star;index_word++){
					//get the topic of current word 
					int k=Parameter.z[d][index_word];
					
					//get the index of current word in total vocabulary
					int w=Document.vocabulary.get(document.wordList.get(index_word));
					
					
					//exclude the current condition from the counters
					Parameter.n_d_k[d][k]--;
					Parameter.n_k[k]--;
					Parameter.n_k_w[k][w]--;
					...
					//resample Z,X
					int[] samplezx=sampleZX(d,index_word,w,document);
					
					Parameter.z[d][index_word]=samplezx[0];
					
					int new_k=Parameter.z[d][index_word];
					
					Parameter.x[d][index_word]=samplezx[1];
					
					//count the new condition into the counters
	...
	//calculate training document's loglikelihood
	...
	//sample ZX
	private static int[] sampleZX(int d, int index_word,int w, Document document){
		
		Double[] potential=new Double[2*Parameter.K];
		double sum_potential=0;
		
		for(int k=0;k<potential.length;k++){
			if(k<Parameter.K){
				double expression1=(1-(double)Parameter.Lambda)*((double)Parameter.n_d_k[d][k]+(double)Parameter.Alpha);
				...
				int tmp_k=k-Parameter.K;
				double expression1=Parameter.Lambda*(double)Parameter.n_d_k[d][tmp_k]+(double)Parameter.Alpha;
				double expression2=(double)document.n_d_star+(double)Parameter.K*(double)Parameter.Alpha;
				double expression3=(double)Parameter.n_c_k_w[document.corpus][tmp_k][w]+(double)Parameter.Beta;
				double expression4=(double)Parameter.n_c_k[document.corpus][tmp_k]+(double)Parameter.V*(double)Parameter.Beta;
				potential[k]=expression1*expression3/(expression2*expression4);	
				sum_potential+=potential[k];				
			}			
		}
		...
		Double[] multiNomial=new Double[potential.length];
		for(int k=0;k<potential.length;k++){
			multiNomial[k]=potential[k]/sum_potential;
		}
		
		//sample z,x from multinomial distribution
	...
	private static void estimateTheta(List TrainingDocumens,int iteration){
		for(int d=0;d<Parameter.N_d;d++){
			...
			for(int k=0;k<Parameter.K;k++){
				...
				//incorporate current theta into final parameter after burnIn
	...
	private static void estimatePhi(List TrainingDocumens,int iteration){
		for(int k=0;k<Parameter.K;k++){
			for(int w=0;w<Parameter.V;w++){
				...
				//incorporate current phi into final parameter after burnIn
				if(iteration>=Parameter.burnIn){
		...
		for(int k=0;k<Parameter.K;k++){
			for(int w=0;w<Parameter.V;w++){
				for(int c=0;c<2;c++){
					...
					//incorporate current phic into final parameter after burnIn
					if(iteration>=Parameter.burnIn){
		...
		for(int iteration=0;iteration<Parameter.iterations;iteration++){
			...
			for(int d=0;d<Parameter.test_N_d;d++){
				...
				for(int index_word=0;index_word<document.n_d_star;index_word++){
					...
					int[] sampleTestZX=sampleTestZX(d,index_word,w,document);
					Parameter.test_z[d][index_word]=sampleTestZX[0];
					int new_k=Parameter.test_z[d][index_word];
					
					Parameter.test_x[d][index_word]=sampleTestZX[1];
	...
	//sample test ZX
	private static int[] sampleTestZX(int d, int index_word,int w, Document document){
		Double[] potential=new Double[2*Parameter.K];
		double sum_potential=0;
		
		for(int k=0;k<potential.length;k++){
			if(k<Parameter.K){
				double expression1=(1-(double)Parameter.Lambda)*((double)Parameter.test_n_d_k[d][k]+(double)Parameter.Alpha);
				double expression2=(double)document.n_d_star+(double)Parameter.K*(double)Parameter.Alpha;
				potential[k]=(expression1/expression2)*Parameter.phi_k_w[k][w];
				sum_potential+=potential[k];
			}else{
				int tmp_k=k-Parameter.K;
				double expression1=Parameter.Lambda*(double)Parameter.test_n_d_k[d][tmp_k]+(double)Parameter.Alpha;
				double expression2=(double)document.n_d_star+(double)Parameter.K*(double)Parameter.Alpha;
				double expression3=(double)Parameter.test_n_c_k_w[document.corpus][tmp_k][w]+(double)Parameter.Beta;
				double expression4=(double)Parameter.test_n_c_k[document.corpus][tmp_k]+(double)Parameter.V*(double)Parameter.Beta;
				potential[k]=expression1*expression3/(expression2*expression4);	
				sum_potential+=potential[k];				
			}			
		}
	...
	private static void outputResults(){
		...
		try {
			...
			PrintStream ps0=new PrintStream(new FileOutputStream(outputDirectory+"/block-output-"+Parameter.K+"-"+Parameter.Lambda+"-"+Parameter.Alpha+".txt-theta"));
			PrintStream ps1=new PrintStream(new FileOutputStream(outputDirectory+"/block-output-"+Parameter.K+"-"+Parameter.Lambda+"-"+Parameter.Alpha+".txt-phi"));
			PrintStream ps2=new PrintStream(new FileOutputStream(outputDirectory+"/block-output-"+Parameter.K+"-"+Parameter.Lambda+"-"+Parameter.Alpha+".txt-phi0"));
			PrintStream ps3=new PrintStream(new FileOutputStream(outputDirectory+"/block-output-"+Parameter.K+"-"+Parameter.Lambda+"-"+Parameter.Alpha+".txt-phi1"));
			PrintStream ps4=new PrintStream(new FileOutputStream(outputDirectory+"/block-output-"+Parameter.K+"-"+Parameter.Lambda+"-"+Parameter.Alpha+".txt-trainll"));
			PrintStream ps5=new PrintStream(new FileOutputStream(outputDirectory+"/block-output-"+Parameter.K+"-"+Parameter.Lambda+"-"+Parameter.Alpha+".txt-testll"));
			PrintStream ps6=new PrintStream(new FileOutputStream(outputDirectory+"/block-output-"+Parameter.K+"-"+Parameter.Lambda+"-"+Parameter.Alpha+".txt-runtime-training"));
			PrintStream ps7=new PrintStream(new FileOutputStream(outputDirectory+"/block-output-"+Parameter.K+"-"+Parameter.Lambda+"-"+Parameter.Alpha+".txt-runtime-test"));
	...
	private static void readParameters(String[] args)
	{
		if (args.length == 9){
			...
			BlockSampler.trainingfile=args[0];
			BlockSampler.testfile=args[1];
			BlockSampler.outputDirectory=args[2];
			...
			BlockSampler.trainingfile="input-train.txt";
			BlockSampler.testfile="input-test.txt";
			BlockSampler.outputDirectory="runtime";
			Parameter.K=25;
